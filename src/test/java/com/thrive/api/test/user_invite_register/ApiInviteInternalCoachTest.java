...
public class ApiInviteInternalCoachTest extends ApiInviteUserCommon{
	
	InviteCoachRequest inviteInternalCoachReqBody;
	Response inviteInternalCoachResponse;
	...
	public void beforeTests() {
		...
		inviteInternalCoachResponse = getInviteInternalCoachResponse(inviteInternalCoachReqBody, headers);
	}
	
	
	@Test(description = "SA - validate invite internal coach api returns successful response message as 200 OK", priority = 0)
	public void validateInviteInternalCoachSuccessfulResponse() {
		...
		Assert.assertEquals(inviteInternalCoachResponse.statusCode(), 200);
		
		getExtentTestLogger().log(Status.PASS, "Then user get successful response message as 200 OK");
		Assert.assertEquals(inviteInternalCoachResponse.statusLine(), "HTTP/1.1 200 OK");
	}
